...
Usage:
    ...
    python run_milestone0.py --json-only
"""
...
from files.models import CriterionResult
...
def print_report(report, evaluations) -> None:
    ...
    print(f"Mean doc confidence:     {report.mean_extraction_confidence:.0%}")
    ...
    for ev in evaluations:
        ...
        print(f"       result:     {ev.result.value} "
...
def main() -> None:
    ...
    out_path = out_dir / f"case_{report.case_id}.json"
    ...
    print(f"Structured output written to: {out_path.relative_to(Path.cwd()) if out_path.is_relative_to(Path.cwd()) else out_path}")
